...
const ViewBlog = () => {
  ...
  const sendRequest = async () => {
    try {
      ...
      const res = await axios.post("http://localhost:8000/api/admin/blogs/add-blog", formData);
  ...
  const modalContent1 = () => {
    ...
    const handleDelete = async (req, res) => {
      try {
        if (uid) {
          await axios.delete(`http://localhost:8000/api/admin/blogs/delete-blog/${uid}`);
        }
        window.location.reload();
      } catch (err) {
        console.log(err);
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="row">
              <div className="col">
                ...
                <div className="input-group mb-3">
                  <select name="type" value={blog.type} onChange={handleChange}>
    ...
    const fetchData = async () => {
      ...
      try {
        const res = await axios.get(
          `http://localhost:8000/api/admin/blogs/get-blog`
        );
...
export default LoadingState(ViewBlog);
