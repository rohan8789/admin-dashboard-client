...
const ViewNews = () => {
  ...
  const sendRequest = async () => {
    try {
      ...
      const res = await axios.post("http://localhost:8000/api/admin/news/post-news",formData);
  ...
  const modalContent1 = () => {
    ...
    const handleDelete = async (req, res) => {
      try {
        if (uid) {
          await axios.delete(`http://localhost:8000/api/admin/news/delete-news/${uid}`);
        }
        window.location.reload();
      } catch (err) {
        console.log(err);
      ...
      <div className="d-flex justify-content-center align-content-center">
        <div className="my-3 p-3 bg-body rounded shadow-sm w-75">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="row">
              <div className="col">
                <label htmlFor="image" className="d-block form-label text-start">
                  Image:
                </label>
                <div className="input-group mb-3">
                  <input
              ...
              <div className="col">
                ...
                <div className="input-group mb-3">
                  <select name="type" value={news.type} onChange={handleChange}>
                    ...
                    <option value="Entertainment News">
                      Entertainment News
                    </option>
  ...
  useEffect(() => {
    const fetchData = async () => {
      // console.log("hello");
      try {
        const res = await axios.get(`http://localhost:8000/api/admin/news/get-news`);
                ...
                {sData?.map((data, i) => {
                  return (
                    <>
                      <tr>
                        <td>{i + 1}</td>
                        <td>{console.log(data?.image)}
                          <img src={data?.image ? 'http://localhost:8000/'+data?.image : "https://via.placeholder.com/150"}
                            alt=""
                            width="32"
                            height="32"
...
export default LoadingState(ViewNews);
